...
<!-- Things outside the scope of this feature but required for it to work. These must already exist or be completed separately before implementation begins.

- Product tags are normalized and stored as arrays in the `products` table
- PostgreSQL full-text search extensions (`tsvector`, `tsquery`) are available in the database
- Existing product list API endpoint is functional and follows REST conventions
-->
